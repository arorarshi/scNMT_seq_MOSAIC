...
lapply(c("data/stage.rmVisceralEnd.Rdata", "data/lineage.all.Rdata", "data/lineage.Rdata","data/cv.stats.stage.noimp.rmViscEnd.Rdata"),load,.GlobalEnv)
```
...
We removed samples belonging to Visceral Endoderms, see lineage table below 

```{r, echo=F}

kable(get.summary(stage, 1, var.n="stage"), "html", caption="sample summary", row.names=TRUE )

kable(table(stage, lineage.all[names(stage)]), "html", caption="stage vs lineage", row.names=TRUE )
...
all.rdata<-c("acc_DHS", "acc_p300", "acc_cgi", "acc_CTCF", "acc_promoter", "acc_genebody", "met_DHS", "met_p300", "met_cgi", "met_CTCF", "met_promoter", "met_genebody", "rna.mat")
...
load("data/solnk.stage.noimp.rmViscEnd.Rdata")
kk = solnk[[13]] 
...
kkAMIlin = AMI(kk, as.character(lineage.nona[names(kk)]))
...
unwtkkAMIlin = AMI(unwtkk$cluster, as.character(lineage.nona[names(unwtkk$cluster)]))
